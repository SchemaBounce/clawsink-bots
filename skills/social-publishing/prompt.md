...
5. Instagram is two-step: `instagram__post_ig_user_media` (creates a non-public container) and `instagram__publish_ig_user_media` (makes it public) are BOTH external actions, both gated, and both need an Inbox approval every time. Publish-class actions can never be auto-approved by policy.

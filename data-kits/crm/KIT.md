...
A focused B2B lead research data kit. Covers the five entity types needed to run a full prospecting workflow: leads, their companies, news signals, outreach templates, and AI-generated research briefs.
...
- **Research Briefs**: AI-generated summaries produced per-lead by a research agent
...
Pairs cleanly with the `saas`, `consulting`, and `financial-ops` data kits. The `crm_` prefix prevents entity collisions. The `research_briefs` entity carries an `agentId` field linking each brief to the agent run that produced it.
